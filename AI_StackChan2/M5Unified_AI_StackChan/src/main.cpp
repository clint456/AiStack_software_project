...
bool init_chat_doc(const char *data)
{
  ...
  serializeJsonPretty(chat_doc, json_str);  // 将字符串打印到串口
...
{
  auto micConfig = M5.Mic.config();   //麦克风模块
  ...
  { /// custom setting
    auto spk_cfg = M5.Speaker.config();  //扬声器模块
...
//  M5.Speaker.begin();
  ...
  {
    uint32_t nvs_handle;
    if (ESP_OK == nvs_open("setting", NVS_READONLY, &nvs_handle)) {   //非易失性存储
      size_t volume;
      uint8_t led_onoff;
      uint8_t speaker_no;
